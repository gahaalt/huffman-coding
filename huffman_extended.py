...
def build_and_encode(text, freq=None, group_size=1):
    get_text = lambda: get_extended_text(text, group_size)

    if freq is None:
        freq = utils.get_symbol_frequency(get_text())
    elif group_size != 1:
        real_freq = utils.get_symbol_frequency(get_text())
    ...
    return huffman_canonical.build_and_encode(get_text(), freq)
